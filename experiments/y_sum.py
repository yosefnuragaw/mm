...
for armsize,round in zip(L_ARMS,L_ROUNDS):
    ...
    for idx, seed in enumerate(SEEDS):
        ...
        alpha = W * np.array(lalpha, dtype=float)
        beta  = W * np.array(lbeta, dtype=float)
        ...
        fPareto = utils.get_fPareto_function(utils.compute_pareto(est_data, 'Cost', 'Score'))
        est_pf = utils.compute_pareto(est_data, 'Cost', 'Score')
        true_pf = utils.compute_pareto(true_data, 'Cost', 'Score')
        igd.append(utils.compute_IGD(true_pf[['Cost', 'Score']].to_numpy(),
        ...
        if idx >= len(SEEDS) - N_PLOT:
            ...
            ax = axes[ax_idx]
            utils.plot_frontier_panel(ax, est_data, true_data, jitter=False, seed=seed, lam=L)
