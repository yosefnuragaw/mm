...
class MessageProcessor:
    ...
    def process_round(self, llm_response, item, messages, conversation_history):
        ...
        if not tool_calls:
            ...
            format_prompt = "Please follow the <tool_call> tag format and return a <tool_call> tag containing function name and parameters. Make sure it can be find with regex : <tool_call>(.*?)</tool_call>. Do not use Markdown. Do not use triple backticks. Return only raw XML-like tags such as <tool_call>...</tool_call>. The output must start directly with <tool_call> and end with </tool_call> so it can be parsed using regex."
    ...
    def parse_query(self, item, content):
        ...
        query = self.parse_tool_calls(preserved_content, item)[0].get("arguments").get("answer")
        return query
